    ...
    const renderAnswerCells = answers.map((answer, index) => {
        return (
            <li className="answer__cell" key={answer.answerId}>
                <svg className={`answer-cell ${answer.isSelected ? "selected" : ""} ${answer.isDisabled ? "disabled" : ""} ${answer.isShown ? "correct" : ""}`} width="320" height="56" viewBox="0 0 320 56" fill="none"
                     xmlns="http://www.w3.org/2000/svg">
                    <path className="answer-cell__body" tabIndex="-1"
                          onClick={() => {scenario(answer.answerId)}}
