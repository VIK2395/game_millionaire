...
const AnswersList = ({ answers, scenario }) => {
  const renderAnswerCells = answers.map((answer, index) => (
    <li className="answer__cell" key={answer.answerId}>
      <svg
        className={`answer-cell ${answer.isSelected ? 'selected' : ''} ${
          answer.isDisabled ? 'disabled' : ''
        } ${answer.isShown ? 'correct' : ''}`}
        width="320"
        height="56"
        viewBox="0 0 320 56"
        fill="none"
  ...
  return <ul className="content__answers-list">{renderAnswerCells}</ul>;
};
