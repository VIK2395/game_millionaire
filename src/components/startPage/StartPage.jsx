...
const StartPage = ({resetEarned, resetScore, setScoreQuestion, updateScoreDashboard, disableIsInitLoad, setIsInGame, isInGame, setIsInGameEnd, formGameQuestions}) => {
		...
		<div className={s.wrapper}>
			<div className={`${s.content} ${s.content_portrait}`}>
				<div className={s['content__portrait-block']}>
					<img className={`${s.content__logo} ${s.logo}`} draggable="false" width="198" height="156"
						 src={logoHand}
						 alt="logo_hand"/>
					<p className={s.content__text}>Who wants to be<br/>a millionaire?</p>
				</div>
				<Link to='/game' className={`${s['content__link-button']} ${s['link-button']}`}
					  onClick={onStartClicked}>Start</Link>
			</div>
